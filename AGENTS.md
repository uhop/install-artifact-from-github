...
- Configuration knobs follow the `--flag` / `--flag-var ENVVAR` / default-env-var triple convention: mirror host (`DOWNLOAD_HOST`), path/version skipping (`DOWNLOAD_SKIP_PATH`, `DOWNLOAD_SKIP_VER`), proxy agent (`DOWNLOAD_AGENT`), N-API level (`DOWNLOAD_NAPI`), forced source build (`DOWNLOAD_FORCE_BUILD` / `--force-build`). The canonical (verified) location is `<host>/<owner>/<repo>/releases/download/`, every part of it parsed from the addon's own `package.json`: `repository.url` names the host when it is a full URL (so an enterprise-hosted addon resolves to its own instance), and `https://github.com` is the fallback for the `owner/repo` / `github:owner/repo` shorthands and for anything unparseable. A `--host` / `DOWNLOAD_HOST` mirror resolving anywhere else is not integrity-checked, while one resolving back onto that prefix still is.
...
- **Every change must be fail-safe**: any failure in the download path must fall back to the consumer's `rebuild` script. Never make a download/verification failure fatal.
- **Integrity verification is opt-in and source-scoped** (since 1.7.0): it runs only for the canonical GitHub source and only when the addon ships an `artifactHashes` bag. The test is whether the _resolved URL_ falls under the addon's own release location — a template over the `host`/`owner`/`repo` parsed from its `package.json`, never a literal — and not whether an override was supplied: the hashes attest to the bytes the author published _there_, so anywhere else (another host, or another path on the same host) is the deployer's own trust root and must never be integrity-checked, while a `--host` / `DOWNLOAD_HOST` resolving back onto that prefix is the original and keeps its check. `GITHUB_SERVER_URL` must never re-enter the host cascade — in GitHub Actions it names the instance running the workflow, not the host of a dependency's artifacts (issue #27). The addon's own `repository.url` is the authority instead, which is also the test harness's seam: a sandbox pointing it at the mock server makes the mock the canonical source. A failed check stays fail-safe (rebuild), never fatal. The bag is authored in the consumer's `package.json`, never in this package.
