...
- **Integrity verification is opt-in and source-scoped** (since 1.7.0): it runs only for the canonical GitHub source and only when the addon ships an `artifactHashes` bag. The test is whether the _resolved URL_ falls under `https://github.com/<owner>/<repo>/releases/download/` — the addon's own release location, templated from its parsed `package.json`, never a literal — and not whether an override was supplied: the hashes attest to the bytes the author published _there_, so anywhere else (another host, or another path on the same host) is the deployer's own trust root and must never be integrity-checked, while a `--host` / `DOWNLOAD_HOST` resolving back onto that prefix is the original and keeps its check. `GITHUB_SERVER_URL` must never re-enter the host cascade — in GitHub Actions it names the instance running the workflow, not the host of a dependency's artifacts (issue #27); `DEVELOPMENT_CANONICAL_HOST` relocates the canonical host and exists solely so the test harness can put it on a local mock. A failed check stays fail-safe (rebuild), never fatal. The bag is authored in the consumer's `package.json`, never in this package.
